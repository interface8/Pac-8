 ...
 const { stats, loading, error } = useTestimonialStats();
  return (
    <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 ">
      ...
      <div className="text-center py-8 rounded-3xl shadow-xs hover:shadow-xl transition-all duration-500 hover:-translate-y-1 border border-[#F05700]/10 bg-white"
           >
      ...
      <div className="text-center py-8 rounded-3xl shadow-xs hover:shadow-xl transition-all duration-500 hover:-translate-y-1 border border-[#F05700]/10 bg-white"
          >
        <div className="text-3xl sm:text-4xl font-semibold mb-6  lg:text-5xl tracking-widest text-orange-600">{loading ? "Loading..." : (stats.averageRating?.toFixed(1) ?? 0)}</div>
